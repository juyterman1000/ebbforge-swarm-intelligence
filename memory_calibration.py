...
def run_calibration(count):
    ...
    swarm = cogops.ProductionTensorSwarm(agent_count=count)
...
if __name__ == "__main__":
    ...
    # We need to recreate the swarm for the extraction test if we want to be precise
    swarm = cogops.ProductionTensorSwarm(agent_count=1_000_000)
    
    # 2. Predictive scaling for 10M
    est_10m = m_1m * 10
    print(f"\n📈 ESTIMATED 10M ACTIVE FOOTPRINT: {est_10m:.2f} MB")
    
    if est_10m > 28000: # 28GB safety limit for 32GB system
        print("⚠️  Warning: 10M active agents will likely OOM.")
    ...
    print(f"   Estimated 10M List Memory: {list_mem * 10:.2f} MB (Would likely crash Python)")
