...
impl AgentGraph {
    ...
    pub async fn run_task(
                    ...
                    Ok(resp) => {
                        if resp.status().is_success() {
                            ...
                            if error.contains("429") || error.contains("RESOURCE_EXHAUSTED") {
                                info!("   [ReAct] ⚠️ Quota exhausted for {} - Falling back immediately...", model);
                                continue;
                            } else if error.contains("Function calling is not enabled") {
                                info!("   [ReAct] ⚠️ {} doesn't support function calling, using text mode", model);
        ...
        if let Some(answer) = &final_answer {
            ...
            info!("⚠️ [AgentGraph] Task ended without calling finish()");
        }
