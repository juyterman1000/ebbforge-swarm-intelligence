...
use super::ebbforge_engine::EbbPool;
...
impl SpatialHashGrid {
    ...
    pub fn rebuild(&mut self, pool: &EbbPool) {
        let n = pool.n_agents;
        ...
        for i in 0..n {
            // Alive logic implicit right now, can add later 
...
mod tests {
    use super::*;
    use crate::swarm::ebbforge_engine::EbbPool;

    fn make_pool_circle(n: usize, radius: f32) -> EbbPool {
        let mut pool = EbbPool::new(n + 1);
        for i in 0..n {
    ...
    fn rebuild_query_counts_match() {
        ...
        // Query from center with radius = world_radius + 1 should return all agents
        let mut found = 0usize;
        grid.query_radius(0.0, 0.0, 8.0, |_| found += 1);
        assert_eq!(found, 200, "expected all agents in radius");
    }
